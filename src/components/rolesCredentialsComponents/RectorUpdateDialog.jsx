...
export const RectorUpdateDialog = ({
  rectors,
  setRectors,
  selectedRector,
  setSelectedRector,
  ...
  const [formData, setFormData] = useState(selectedRector); // Initialize with student data
  ...
  const filterRectors = (formData) => {
    const filteredRectors = [];
    rectors.map((rector) => {
      if (rector.rector_id === selectedRector.rector_id) {
        filteredRectors.push(formData);
      } else {
        filteredRectors.push(rector);
      }
    });
    setRectors(filteredRectors);
  };
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        toast.success('Rector data updated successfully');
        setSelectedRector(formData);
        filterRectors(formData);
      } else {
        toast.error('Error updating Rector details');
      }
    } catch (error) {
      console.error('Error updating Rector details:', error);
      toast.error('Error updating Rector details');
    } finally {
      setLoading(false);
    ...
    <>
      ...
      {currentPage === 1 && (
        <Dialog open={openEditDialog} onClose={() => setOpenEditDialog(false)}>
          <DialogTitle>Edit Rector Details</DialogTitle>
          <DialogContent>
