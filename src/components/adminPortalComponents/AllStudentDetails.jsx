...
import { faSearch, faChevronDown, faChevronUp } from '@fortawesome/free-solid-svg-icons'; // Import FontAwesome icons
...
const AllStudentDetails = () => {
    ...
    const searchByPinNumber = async () => {
        ...
        try {
            const response = await axios.get('http://localhost:5000/api/student/studentDetailsByPinNumber', {
                params: {
    ...
    return (
        <div className="text-black">
            <div className="p-4">
                ...
                />
                <button onClick={searchByPinNumber} className="ml-4 bg-blue-500 text-white px-4 py-2 rounded-xl">
            ...
            <div className="w-full text-black p-4">
                {studentData.map((student, index) => (
                    <div key={index} className="rounded-lg border border-gray-700 mb-4 bg-blue-200">
                        {/* List Tile Section */}
                        <div className="flex justify-between items-center p-2">
                            <div className="flex items-start">
                                ...
                                <div className="ml-4 flex flex-col items-start">
                                    <h3 className="font-semibold text-lg">{student.student_full_name || 'Unknown'}</h3>
                                    <p className="text-sm text-gray-600">PIN: {student.pin_number || 'Null'}</p>
                                </div>
                            </div>
                        ...
                        <div className={`transition-all duration-300 ${visibleCards[index] ? 'max-h-screen overflow-auto' : 'max-h-0 overflow-hidden'}`}>
                            <UpdatedCard student={student} index={index} />
                        </div>
                    </div>
                ))}
